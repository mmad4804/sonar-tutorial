...
class CustomFileReaderTest {
  ...
  void testFalseEquals() {
    CustomFileReader diffSentence = new CustomFileReader("readMe1.txt");
    diffSentence.setNewSentence("Something different");
    assertNotEquals(sut, diffSentence);

    CustomFileReader diffCount = new CustomFileReader("readMe1.txt");
    try {
      diffCount.howManyWordsInFile();
    } catch (FileNotFoundException e) {
      fail("File should exist");
    }
    assertNotEquals(sut, diffCount);

    CustomFileReader diffPath = new CustomFileReader("other.txt");
    assertNotEquals(sut, diffPath);
  }
}